...
import { mockAllocationLog, mockTeams } from '../mockData';

const AllocationLogSection = () => {
  return (
    <Card>
      <h2 className="text-xl font-semibold text-slate-800 mb-4">📋 Allocation Log</h2>
      <div className="h-96 overflow-y-auto">
        <table className="min-w-full">
          <thead>
            <tr>
              <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Lead</th>
              <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Routing Rationale</th>
              <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Assigned To</th>
              <th className="px-4 py-2 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Status & Outcome</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200/50">
            {mockAllocationLog.map(log => {
              const rep = mockTeams.flatMap(t => t.members).find(m => m.id === log.assignedToId);
              ...
              return (
                <tr key={log.id}>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-sm text-slate-600">{rep ? rep.name : 'N/A'}</td>
...
export default AllocationLogSection;