export function AdminView(): HTMLElement {
  const el = document.createElement('div');
  el.className = 'grid grid-cols-1 lg:grid-cols-3 gap-8';

  const mainCol = document.createElement('div');
  mainCol.className = 'lg:col-span-2 space-y-8';
  mainCol.appendChild(Card('🚀 Live Pop-up Simulation', 'Test the booking flow.'));
  mainCol.appendChild(Card('📋 Allocation Log', 'Recent lead routing outcomes.'));
  mainCol.appendChild(Card('⏰ Team Working Hours', 'Define working hours for teammates.'));
  mainCol.appendChild(Card('🧠 Routing Rules', 'High-level rule overview.'));

  const rightCol = document.createElement('div');
  rightCol.className = 'lg:col-span-1 space-y-8';
  rightCol.appendChild(Card('📊 Live Allocation Queue', 'Who is next to receive leads.'));

  el.appendChild(mainCol);
  el.appendChild(rightCol);
  return el;
}

function Card(title: string, body: string): HTMLElement {
  const el = document.createElement('div');
  el.className = 'glass-card';
  el.innerHTML = `
    <h2 class="text-xl font-semibold text-slate-800 mb-2">${title}</h2>
    <p class="text-sm text-slate-500">${body}</p>
  `;
  return el;
}
